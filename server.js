...
function parseRoster(text, employees) {
  ...
  for (const rawLine of text.split(/[\n;]+/)) {
    ...
    for (const tok of tokens) {
      ...
      if (isFirst || isSecond) {
        if (name) { out.push({ day, kind: isFirst ? "day" : "night", name }); name = null; }
        continue;
      }
...
if (BOT_TOKEN) {
  ...
  bot.use(async (ctx, next) => {
    if (!adminIds.length) return ctx.reply("⚠️ Бот открыт для всех: переменная ADMIN_ID не заполнена. Узнай свой id командой /id и впиши его на Render.");
    if (!adminIds.includes(String(ctx.from?.id))) return ctx.reply("Доступ закрыт. Твой id: " + ctx.from?.id);
    await next();
  });
